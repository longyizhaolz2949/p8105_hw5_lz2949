...
```
Description: From the second plot, the average value of mu_hat is very close to the true value of mu. For example, when the true value of mu is 1, the avergae mu_hat is 1.01; when the true value is 5, the average mu_hat is 5.02. In the third plot, at larger mu values (4,5,6), the average estimated mu_hat values are very close to true values. However, at smaller mu values, the deviation is larger. For example, when the true mu value is 1, the average estimated mu_hat is 2.27; when true mu value is 3, the average estimated mu_hat is 3.20. 

When the null is rejected, it means there is evidence suggest that true mu is different from tested value. So plot 3's values are not approximately equal. 
